            ...
            if "qualifiers" in property[serial]:
                ...
                for pid in qualifiers.keys():
                    for i in range(0,len(qualifiers[pid])):
                        ...
                        if "datavalue" in qualifiers[pid][i]:
                            q_datavalue = qualifiers[pid][i]["datavalue"]["value"]
                            insert_datavalue(q_data_type, q_datavalue, q_hash)
                        # cursor.execute("LOCK TABLES qualifier WRITE")
                        cursor.execute("INSERT INTO qualifier(hash, snak_id, snaktype, property_id, datatype) VALUES (%s, %s, %s, %s, %s)", (q_hash, snak_id, q_snak_type, q_property_id, q_data_type))
                        # cursor.execute("UNLOCK TABLES")
...
def insert_datavalue(datavalue_type, datavalue_value, snak_id):
    if datavalue_type == "string":
        ...
        cursor.execute("INSERT IGNORE INTO datavalue_string VALUES (%s, %s)", (snak_id, datavalue_value))
        # cursor.execute("UNLOCK TABLES")
        ...
        cursor.execute("INSERT IGNORE INTO datavalue_wikibase VALUES (%s, %s)", (snak_id, wikibase_entityid))
        # cursor.execute("UNLOCK TABLES")
        ...
        cursor.execute("INSERT IGNORE INTO datavalue_time VALUES (%s, %s, %s, %s, %s, %s, %s)", (snak_id, time, timezone, before, after, precision, calendarmodel))
        # cursor.execute("UNLOCK TABLES")
        ...
        cursor.execute("INSERT IGNORE INTO datavalue_globecoordinate VALUES (%s, %s, %s, %s, %s, %s)", (snak_id, latitude, longitude, altitude, precision, globe))
        # cursor.execute("UNLOCK TABLES")
        ...
        cursor.execute("INSERT IGNORE INTO datavalue_quantity VALUES (%s, %s, %s, %s, %s)", (snak_id, amount, upperBound, lowerBound, unit))
        # cursor.execute("UNLOCK TABLES")
...
if __name__ == '__main__':
    ...
    print("start insert")
    begin = 1
    end = 1000000 #end is not included
    f = open("wikidata-latest-all.json","r")
