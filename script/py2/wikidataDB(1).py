...
def insert():
    ...
    cursor.execute("INSERT INTO entity VALUES (null, %s, %s, %s, %s)", (entity_id, entity_lang, entity_type, entity_text))
    cursor.execute("INSERT INTO description VALUES (null, %s, %s, %s)", (entity_id, desc_lang, entity_desc))

    for key in claims.keys():
        property_id = key
        property = claims[key]
        for serial in xrange(0,len(property)):  
            ...
            if "qualifiers" in property[serial]:
                ...
                for pid in qualifiers.keys():
                    for i in xrange(0,len(qualifiers[pid])):
                        q_hash = qualifiers[pid][i]["hash"]
                        q_snak_type = qualifiers[pid][i]["snaktype"]
                        q_property_id = pid
                        q_data_type = qualifiers[pid][i]["datatype"]
                        cursor.execute("INSERT INTO qualifier VALUES (%s, %s, %s, %s)", (q_hash, q_snak_type, q_property_id, q_data_type))
            ...
            if datavalue_type == "string":
                ...
                cursor.execute("INSERT INTO datavalue_quantity VALUES (%s, %s, %s, %s)", (snak_id, amount, upperBound, lowerBound))
